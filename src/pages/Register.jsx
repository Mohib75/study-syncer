					...
					<div className='w-full max-w-md p-8 space-y-3 rounded-xl drop-shadow-xl h-full bg-gray-500 bg-clip-padding backdrop-filter backdrop-blur-xl bg-opacity-20 border border-gray-100'>
						...
						<form action='' className='space-y-6' onSubmit={handleSubmit(onSubmit)}>
							...
							<div className='space-y-1 text-sm relative'>
								...
								<span
									className='absolute bottom-4 right-4 cursor-pointer text-gray-900'
									onClick={() => setShowPassword(!showPassword)}>
									{showPassword ? <FaEyeSlash /> : <FaEye />}
								</span>
							</div>
							<button
								className='block w-full p-3 text-center text-white bg-transparent border-solid border-[1px] border-[#FFFFFF45] hover:bg-[#FFFFFF45] transition-all duration-500 rounded-md'
								type='submit'>
								Register
							</button>
						</form>
